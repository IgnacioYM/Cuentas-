...
// ─── Validación post-IA ───────────────────────────────────────────────────────
// Red de seguridad: corrige errores conocidos ANTES de guardar
const validateInvoice = (inv, filename) => {
  ...
  // 1. ARENA ASSET MANAGEMENT → siempre AN, Comisión de gestión, sin retención
  if (prov.includes("arena asset") || prov.includes("arena am")) {
    v.activo = "AN"; v.categoria = "Comisión de gestión"; v.otros = null;
  }

  // 2. ALZAI → AN siempre. Opex si base<=200, Costes implementación si Mod600 o base>500
  ...
  // 5. CONSELL DE MALLORCA → BB5, Capex, sin IVA
  if (prov.includes("consell de mallorca")) {
    v.activo = "BB5"; v.categoria = "Capex"; v.iva = 0; v.otros = null;
  }

  // 6. AJUNTAMENT — ICIO o serveis urbanístics en Barrera → BB5, Capex, sin IVA
  ...
  // 9. FERRETERÍA LA CENTRAL → Capex, BB5 por defecto salvo anotación
  if (prov.includes("ferretería") || prov.includes("ferreteria") || prov.includes("central de san mag")) {
    v.categoria = "Capex";
    if (!v.activo) v.activo = "BB5";
  }

  // 10. RICARDO RUIZ RUBIO → Capex, activo por nombre archivo
  if (prov.includes("ricardo ruiz")) {
    ...
    else v.activo = ""; // revisión manual
  }

  // 11. VÍCTOR CUENCA → Costes legales
  ...
  // 12. RAMOS ORTIZ → AN, Costes implementación (préstamos/avales)
  if (prov.includes("ramos ortiz")) {
    v.activo = "AN"; v.categoria = "Costes de implementación (SPV, abogados, etc.)";
  }

  // 13. NOTARÍA CV INMUEBLES — valor alto (~623k) → vacío para split
  if ((prov.includes("barrios") || prov.includes("notari") || prov.includes("alcala 35")) && concepto.includes("compraventa")) {
    ...
    // Si el total es muy alto (>500k incluidos gastos), son múltiples inmuebles
    if ((v.cuantia||0) > 900 || totalCV > 1200) {
      // Verificar si es un solo inmueble o varios
      if ((v.cuantia||0) > 900) {
        v.activo = "";
  ...
  // 14. NOTARÍA — forzar cuantia = BI + suplidos si detectamos base no sujeta
  // (esto no podemos validar sin el PDF, pero al menos verificamos coherencia)
  ...
  return (
    ...
    >
      ...
      {files.length > 0
        ...
        : <div><div style={{fontSize:32,marginBottom:10}}>📄</div><div style={{color:"#64748b",fontSize:14}}>Arrastra facturas PDF</div><div style={{color:"#334155",fontSize:12,marginTop:4}}>o haz clic para seleccionar</div></div>}
    </div>
  );
}
  ...
  return (
    <div style={{ background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:12, overflow:"hidden" }}>
      <div style={{ padding:"8px 14px", borderBottom:"1px solid #1e3a5f", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <span style={{ fontSize:11, color:"#475569", letterSpacing:1, textTransform:"uppercase" }}>Factura original</span>
...
export default function App() {
  ...
  // Ref para deduplicar sin depender del estado en closures
  ...
  const resetReview = () => {
    setDraft(null);
    setSplitMode(false);
    setSplits([{activo:"",pct:50},{activo:"",pct:50}]);
  };

  // Deduplicar usando ref (siempre actualizado)
  const addInvoices = (newInvs) => {
    ...
    if (dupes > 0 && fresh.length === 0) {
      showFlash("Factura(s) ya registrada(s), omitidas.", "warn");
      return;
    }
  ...
  const callAPI = async (base64, filename, retries=2) => {
    for (let attempt=0; attempt<=retries; attempt++) {
      try {
        ...
        return validateInvoice(JSON.parse(clean), filename);
      } catch(e) {
  ...
  const confirmInvoice = () => {
    if (!draft.fecha || !draft.proveedor || !draft.categoria || !draft.cuantia) {
      showFlash("Completa los campos obligatorios.","err"); return;
    }
    if (splitMode) {
      ...
      const newInvs = splits.map((s,i) => ({
        ...
        comentario: draft.comentario ? `[${s.pct}% de factura original] ${draft.comentario}` : `${s.pct}% de factura original`,
      }));
      addInvoices(newInvs);
    } else {
  ...
  const exportCSV = () => {
    ...
    const rows = invoices.map(e => [
      ...
      (e.cuantia||0).toFixed(2).replace(".",","),
      (e.iva||0).toFixed(2).replace(".",","),
      (e.otros||0).toFixed(2).replace(".",","),
      ((e.cuantia||0)+(e.iva||0)).toFixed(2).replace(".",","),
  ...
  if (!loaded) return (
    <div style={{ background:"#080f1a", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", color:"#3b82f6", fontFamily:"monospace" }}>
      Cargando…
    </div>
  );
    ...
    <div style={{ background:"#080f1a", minHeight:"100vh", fontFamily:"'DM Mono','Fira Code',monospace", color:"#e2e8f0" }}>
      ...
      {flash && (
        <div style={{ position:"fixed", top:20, right:20, zIndex:999,
          ...
          padding:"10px 18px", borderRadius:8, fontSize:13, maxWidth:380, boxShadow:"0 4px 20px rgba(0,0,0,0.5)"
        }}>
          {flash.msg}
        </div>
      )}
      ...
      {showApiModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ background:"#0d1f35", border:"1px solid #1e3a5f", borderRadius:12, padding:28, width:420, maxWidth:"90vw" }}>
            ...
            <div style={{ fontSize:12, color:"#475569", marginBottom:16 }}>Necesaria para analizar PDFs. Se guarda solo en este navegador.</div>
            <input type="password" placeholder="sk-ant-..." defaultValue={apiKey} id="api-key-input"
              style={{ width:"100%", background:"#080f1a", border:"1px solid #1e3a5f", borderRadius:6, color:"#e2e8f0", padding:"10px 12px", fontFamily:"inherit", fontSize:13, outline:"none", boxSizing:"border-box", marginBottom:16 }} />
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>saveApiKey(document.getElementById("api-key-input").value)}
                style={{ background:"#1d4ed8", border:"none", color:"#fff", padding:"8px 20px", borderRadius:6, cursor:"pointer", fontSize:13, fontFamily:"inherit", fontWeight:600 }}>Guardar</button>
              <button onClick={()=>setShowApiModal(false)}
                style={{ background:"transparent", border:"1px solid #334155", color:"#64748b", padding:"8px 16px", borderRadius:6, cursor:"pointer", fontSize:13, fontFamily:"inherit" }}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ borderBottom:"1px solid #1e3a5f", padding:"22px 32px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:16 }}>
          ...
          <div style={{ textAlign:"right" }}>
            <div style={{ fontSize:11, color:"#64748b", marginBottom:2 }}>{entries.length} mov. · {invoices.length} facturas</div>
            <div style={{ fontSize:20, fontWeight:700, color:"#f87171" }}>{fmt(total)}</div>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display:"flex", borderBottom:"1px solid #1e3a5f", padding:"0 32px" }}>
        {TABS.map(([key,label]) => (
          <button key={key} onClick={()=>setTab(key)}
            style={{ background:"none", border:"none", cursor:"pointer", padding:"12px 20px", fontSize:13, fontFamily:"inherit",
      ...
      <div style={{ padding:"24px 32px" }}>
        ...
        {tab==="facturas" && (
          <div>
            <div style={{ display:"flex", gap:24, flexWrap:"wrap", alignItems:"flex-start" }}>
              ...
              {/* Columna izq: subir PDFs */}
              <div style={{ flex:"0 0 280px" }}>
                <div style={{ fontSize:12, color:"#475569", marginBottom:16, lineHeight:1.7 }}>
                  Arrastra facturas PDF. Claude las analiza y solo te pide revisión cuando tiene dudas.
                </div>
                <PdfDropZone files={pdfFiles} disabled={analyzing}
                  onFiles={newFiles => setPdfFiles(prev => {
                    const names = new Set(prev.map(f=>f.name));
                    return [...prev, ...newFiles.filter(f=>!names.has(f.name))];
                  })} />

                {pdfFiles.length > 0 && !draft && (
                  <button onClick={analyzeAll} disabled={analyzing}
                    style={{ marginTop:14, width:"100%", background:analyzing?"#1e3a5f":"#1d4ed8", border:"none", color:"#fff", padding:"11px", borderRadius:8, cursor:analyzing?"default":"pointer", fontSize:14, fontFamily:"inherit", fontWeight:600 }}>
                    {analyzing ? `Analizando… (${progress.current}/${progress.total})` : `✦ Analizar ${pdfFiles.length>1?pdfFiles.length+" facturas":"factura"} con IA`}
                  </button>
                )}

                {analyzing && progress.total > 0 && (
                  <div style={{ marginTop:8 }}>
                    <div style={{ background:"#1e3a5f", borderRadius:4, height:4, overflow:"hidden" }}>
                      <div style={{ background:"#3b82f6", height:"100%", borderRadius:4, transition:"width 0.4s", width:`${(progress.current/progress.total)*100}%` }} />
                    </div>
                    <div style={{ fontSize:11, color:"#475569", marginTop:4, textAlign:"center" }}>
                      {progress.current < progress.total ? `Lote ${Math.ceil((progress.current||1)/4)} de ${Math.ceil(progress.total/4)}` : "Finalizando…"}
                    </div>
                  </div>
                )}

                {reviewQueue.length > 0 && draft && (
                  <div style={{ marginTop:10, padding:"8px 12px", background:"#1c1917", border:"1px solid #f59e0b", borderRadius:8, fontSize:12, color:"#fbbf24" }}>
                    ⚠ Revisión {reviewIdx+1} de {reviewQueue.length}
                  </div>
                )}
              ...
              {/* Columna centro: visor PDF */}
              {draft && draft._file && (
                <div style={{ flex:"1 1 360px", minWidth:300, maxWidth:500 }}>
                  <PdfViewer file={draft._file} />
                </div>
              )}

              {/* Columna der: formulario revisión */}
              {draft && (
                <div style={{ flex:"1 1 320px", minWidth:280, background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:12, padding:20 }}>
                  <div style={{ fontSize:13, color:"#3b82f6", marginBottom:16, fontWeight:600 }}>
                    Revisa y confirma
                    {draft.notas && draft.notas.trim() && (
                      <div style={{ marginTop:6, padding:"8px 12px", background:"#1c1917", border:"1px solid #f59e0b", borderRadius:6, color:"#fbbf24", fontSize:12, fontWeight:400 }}>⚠ {draft.notas}</div>
                    )}
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 16px" }}>
                    ...
                    {!splitMode
                      ? <Field label="Activo" value={draft.activo} onChange={v=>setDraft(d=>({...d,activo:v}))} required options={ACTIVOS} />
                      : <div style={{ display:"flex", alignItems:"center", fontSize:11, color:"#475569" }}>Activo → ver reparto ↓</div>
                    }
                    ...
                    <div style={{ color:"#94a3b8", fontSize:13, display:"flex", alignItems:"center", gap:8 }}>
                      Total: <span style={{ color:"#f87171", fontWeight:700 }}>{fmt((draft.cuantia||0)+(draft.iva||0)+(draft.otros||0))}</span>
                    </div>
                  </div>
                  ...
                  <div style={{ marginTop:4, marginBottom:16 }}>
                    ...
                    <textarea value={draft.comentario||""} onChange={e=>setDraft(d=>({...d,comentario:e.target.value}))}
                      placeholder="Aclaraciones, contexto, notas…" rows={2}
                      style={{ width:"100%", background:"#080f1a", border:"1px solid #1e3a5f", borderRadius:6, color:"#94a3b8", padding:"8px 10px", fontFamily:"inherit", fontSize:12, outline:"none", resize:"vertical", boxSizing:"border-box" }} />
                  </div>

                  {/* Toggle split */}
                  <div style={{ marginBottom:14, borderTop:"1px solid #1e3a5f", paddingTop:14, display:"flex", alignItems:"center", gap:10, cursor:"pointer" }}
                    onClick={()=>setSplitMode(v=>!v)}>
                    <div style={{ width:36, height:20, borderRadius:10, background:splitMode?"#1d4ed8":"#1e3a5f", position:"relative", transition:"background 0.2s", flexShrink:0 }}>
                      <div style={{ position:"absolute", top:3, left:splitMode?18:3, width:14, height:14, borderRadius:"50%", background:splitMode?"#93c5fd":"#475569", transition:"left 0.2s" }} />
                    </div>
                    <span style={{ fontSize:12, color:splitMode?"#93c5fd":"#475569", userSelect:"none" }}>Repartir entre varios activos</span>
                  </div>

                  {splitMode && (
                    <div style={{ background:"#080f1a", border:"1px solid #1e3a5f", borderRadius:10, padding:14, marginBottom:16 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                        <span style={{ fontSize:11, color:"#475569", letterSpacing:1, textTransform:"uppercase" }}>Distribución</span>
                        <div style={{ display:"flex", gap:8 }}>
                          <button onClick={()=>{ const p=parseFloat((100/splits.length).toFixed(4)); setSplits(prev=>prev.map((s,i)=>({...s,pct:i===prev.length-1?100-p*(prev.length-1):p}))); }}
                            style={{ background:"transparent", border:"1px solid #1e3a5f", color:"#64748b", padding:"3px 10px", borderRadius:5, cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>Igualar %</button>
                          <button onClick={()=>setSplits(prev=>[...prev,{activo:"",pct:0}])}
                            style={{ background:"transparent", border:"1px solid #1e3a5f", color:"#64748b", padding:"3px 10px", borderRadius:5, cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>+ Añadir</button>
                        </div>
                      </div>
                      {splits.map((s,i) => (
                        <SplitRow key={i} split={s} index={i} count={splits.length}
                          base={draft.cuantia} iva={draft.iva} otros={draft.otros}
                          onChange={(idx,key,val)=>setSplits(prev=>prev.map((sp,k)=>k===idx?{...sp,[key]:val}:sp))}
                          onRemove={idx=>setSplits(prev=>prev.filter((_,k)=>k!==idx))} />
                      ))}
                      <div style={{ marginTop:8, display:"flex", justifyContent:"space-between", fontSize:12 }}>
                        <span style={{ color:Math.abs(totalPct-100)<0.01?"#4ade80":"#f87171" }}>
                          {totalPct.toFixed(1)}% {Math.abs(totalPct-100)<0.01?"✓":`(falta ${(100-totalPct).toFixed(1)}%)`}
                        </span>
                  ...
                  <div style={{ display:"flex", gap:10 }}>
                    <button onClick={confirmInvoice}
                      style={{ background:"#052e16", border:"1px solid #22c55e", color:"#4ade80", padding:"9px 20px", borderRadius:8, cursor:"pointer", fontSize:13, fontFamily:"inherit", fontWeight:600 }}>
                      ✓ Confirmar y guardar
                    </button>
                    <button onClick={advanceReview}
                      style={{ background:"transparent", border:"1px solid #334155", color:"#64748b", padding:"9px 14px", borderRadius:8, cursor:"pointer", fontSize:13, fontFamily:"inherit" }}>
                      {reviewQueue.length > 1 ? "Saltar" : "Cancelar"}
                    </button>
                  </div>
                </div>
              )}
            ...
            {invoices.length > 0 && (
              <div style={{ marginTop:32 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                  <div style={{ fontSize:13, color:"#64748b" }}>{invoices.length} facturas registradas</div>
                  <div style={{ display:"flex", gap:8 }}>
                    ...
                    <tbody>
                      {invoices.map((inv,i) => (
                        <tr key={inv.id} style={{ background:i%2===0?"#0a1628":"#080f1a" }}
                          ...
                          <td style={{ padding:"8px 10px", textAlign:"right", color:"#64748b" }}>{fmt(inv.iva||0)}</td>
                          <td style={{ padding:"8px 10px", textAlign:"right", color:inv.otros?"#f59e0b":"#334155" }}>{inv.otros ? fmt(inv.otros) : "—"}</td>
                          <td style={{ padding:"8px 10px", textAlign:"right", color:"#94a3b8" }}>{fmt((inv.cuantia||0)+(inv.iva||0))}</td>
                          <td style={{ padding:"8px 10px", textAlign:"right", color:"#f87171", fontWeight:600 }}>{fmt((inv.cuantia||0)+(inv.iva||0)+(inv.otros||0))}</td>
                          <td style={{ padding:"8px 10px", color:"#475569", fontSize:11, maxWidth:140, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }} title={inv.comentario||""}>{inv.comentario||"—"}</td>
                            ...
                            {delConfirm===inv.id
                              ? <span>
                                  <button onClick={()=>{ 
                                    ...
                                    style={{ background:"#450a0a", border:"1px solid #ef4444", color:"#f87171", padding:"2px 7px", borderRadius:4, cursor:"pointer", fontSize:11, fontFamily:"inherit", marginRight:4 }}>Sí</button>
                                  <button onClick={()=>setDelConfirm(null)}
                                    style={{ background:"transparent", border:"1px solid #334155", color:"#64748b", padding:"2px 7px", borderRadius:4, cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>No</button>
                                </span>
                              : <button onClick={()=>setDelConfirm(inv.id)}
                                  style={{ background:"transparent", border:"none", color:"#334155", cursor:"pointer", fontSize:13, padding:"2px 5px" }}>✕</button>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
              {cats.map(c => (
                <button key={c} onClick={()=>setFilterCat(c)}
                  style={{ background:filterCat===c?"#1e3a5f":"transparent", border:`1px solid ${filterCat===c?"#3b82f6":"#1e3a5f"}`, color:filterCat===c?"#93c5fd":"#64748b", padding:"5px 12px", borderRadius:20, cursor:"pointer", fontSize:12, fontFamily:"inherit" }}>
                  {c}
                </button>
              ))}
            </div>
            {filtered.length === 0
              ? <div style={{ textAlign:"center", color:"#334155", padding:"60px 0", fontSize:14 }}>No hay movimientos. Ve a <span style={{ color:"#3b82f6", cursor:"pointer" }} onClick={()=>setTab("gf-importar")}>Importar</span>.</div>
              : <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                    ...
                    <tbody>
                      {filtered.map((e,i) => {
                        const col = CAT_COLORS[e.cat]||{bg:"#111",accent:"#888"};
                        return (
                            ...
                            <td style={{ padding:"9px 8px", textAlign:"center" }}>
                              {delConfirm===e.id
                                ? <span style={{ fontSize:11 }}>
                                    <button onClick={()=>{ setEntries(p=>p.filter(x=>x.id!==e.id)); setDelConfirm(null); showFlash("Eliminado.","warn"); }}
                                      style={{ background:"#450a0a", border:"1px solid #ef4444", color:"#f87171", padding:"2px 8px", borderRadius:4, cursor:"pointer", fontSize:11, fontFamily:"inherit", marginRight:4 }}>Sí</button>
                                    <button onClick={()=>setDelConfirm(null)}
                                      style={{ background:"transparent", border:"1px solid #334155", color:"#64748b", padding:"2px 8px", borderRadius:4, cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>No</button>
                                  </span>
                                : <button onClick={()=>setDelConfirm(e.id)}
                                    style={{ background:"transparent", border:"none", color:"#334155", cursor:"pointer", fontSize:14, padding:"2px 6px" }}>✕</button>}
                            </td>
                          </tr>
                        );
                      })}
        ...
        {tab==="gf-resumen" && (
          <div style={{ maxWidth:540 }}>
            ...
            {Object.entries(summary).map(([cat,val]) => {
              const col = CAT_COLORS[cat]||{accent:"#888"};
              const pct = total!==0 ? (val/total)*100 : 0;
              const subs = entries.filter(e=>e.cat===cat).reduce((acc,e)=>{ acc[e.sub]=(acc[e.sub]||0)+e.amount; return acc; }, {});
              return (
                <div key={cat} style={{ marginBottom:20 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                    <span style={{ color:col.accent, fontSize:13 }}>{cat}</span>
                    <span style={{ color:"#f87171", fontWeight:700 }}>{fmt(val)}</span>
                  </div>
                  <div style={{ background:"#0d1f35", borderRadius:4, height:6, overflow:"hidden" }}>
                    <div style={{ background:col.accent, width:`${Math.abs(pct)}%`, height:"100%", borderRadius:4 }} />
                  </div>
                  <div style={{ marginTop:8, paddingLeft:12, borderLeft:`2px solid ${col.accent}30` }}>
            ...
            {entries.length > 0 && (
              <div style={{ marginTop:24, padding:14, background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:8, fontSize:12, color:"#475569" }}>
                Período: <span style={{ color:"#94a3b8" }}>{entries[0]?.date}</span> → <span style={{ color:"#94a3b8" }}>{entries[entries.length-1]?.date}</span> · {entries.length} movimientos
              </div>
            )}
          </div>
        )}
        ...
        {tab==="gf-importar" && (
          <div style={{ maxWidth:720 }}>
            <div style={{ fontSize:13, color:"#475569", marginBottom:24, lineHeight:1.7 }}>Arrastra los Excel de extractos bancarios.</div>
            ...
            <div style={{ display:"flex", gap:10, alignItems:"center", marginTop:8 }}>
              ...
              {entries.length > 0 && (
                <button onClick={()=>{ setEntries([]); showFlash("Gastos financieros eliminados.","err"); }}
                  style={{ background:"transparent", border:"1px solid #450a0a", color:"#f87171", padding:"9px 16px", borderRadius:8, cursor:"pointer", fontSize:13, fontFamily:"inherit" }}>
                  Borrar todo
                </button>
              )}
            </div>
